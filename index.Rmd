...
##Additional graph showing total number of districts flipped and not flipped.
class(gerrymandering$flipped_district)
names(gerrymandering$flipped_district) <- c("Not Flipped", "Flipped")

ggplot(data = gerrymandering,
       mapping = aes(x = flipped_district)) +
  geom_bar() +
  scale_x_discrete(labels = c("Not Flipped", "Flipped")) +
  labs(x = "Flip of District", y = "Number of Districts", title = "Congressional Districts that Flipped in the 2018 House Elections", fill = "District")
...
```{r}
gerrymandering$flip18 <- as.factor(gerrymandering$flip18)
class(gerrymandering$flip18)
levels(gerrymandering$flip18)

library(descr) 
CrossTable(x = gerrymandering$gerry_new, y = gerrymandering$flip18,
